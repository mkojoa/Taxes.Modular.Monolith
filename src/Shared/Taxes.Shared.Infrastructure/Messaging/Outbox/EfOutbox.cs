...
internal sealed class EfOutbox<T> : IOutbox where T : DbContext
{
    ...
    public async Task PublishUnsentAsync()
    {
        ...
        foreach (var outboxMessage in unsentMessages)
        {
            ...
            _messageContextRegistry.Set(message, new MessageContext(messageId, new Context(correlationId, outboxMessage.TraceId, new IdentityContext(outboxMessage.UserId))));
